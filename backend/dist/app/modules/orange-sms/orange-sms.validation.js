"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.sendOrangeSmsTestValidationSchema = exports.updateOrangeSmsValidationSchema = void 0;
const zod_1 = require("zod");
exports.updateOrangeSmsValidationSchema = zod_1.z.object({
    body: zod_1.z
        .object({
        clientId: zod_1.z.string().trim().min(1, 'Client ID cannot be empty').optional(),
        clientSecret: zod_1.z
            .string()
            .trim()
            .min(8, 'Client secret must be at least 8 characters')
            .optional(),
        senderAddress: zod_1.z
            .string()
            .trim()
            .regex(/^\+?\d+$/, 'Sender address must contain digits and optional leading +')
            .optional(),
        senderName: zod_1.z
            .string()
            .trim()
            .max(11, 'Sender name must be 11 characters or fewer')
            .optional(),
        countryCode: zod_1.z
            .string()
            .trim()
            .regex(/^\d{1,4}$/, 'Country code must be numeric')
            .optional(),
    })
        .refine((data) => Object.values(data).some((value) => value !== undefined && value !== null), {
        message: 'At least one field must be provided to update Orange SMS configuration',
        path: [],
    }),
});
exports.sendOrangeSmsTestValidationSchema = zod_1.z.object({
    body: zod_1.z
        .object({
        phoneNumber: zod_1.z
            .string()
            .trim()
            .regex(/^[+]?\d{6,20}$/u, 'Phone number must include country code and digits only'),
        message: zod_1.z.string().trim().min(1, 'Message is required').max(500),
        senderName: zod_1.z.string().trim().max(11).optional(),
        clientId: zod_1.z.string().trim().optional(),
        clientSecret: zod_1.z.string().trim().optional(),
    })
        .superRefine((data, ctx) => {
        if ((data.clientId && !data.clientSecret) || (!data.clientId && data.clientSecret)) {
            ctx.addIssue({
                code: zod_1.z.ZodIssueCode.custom,
                message: 'Both clientId and clientSecret are required when testing with overrides',
            });
        }
    }),
});
//# sourceMappingURL=orange-sms.validation.js.map