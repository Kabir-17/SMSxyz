...
export interface IAttendanceMethods {
    ...
    updateStudentStatus(studentId: string, status: 'present' | 'absent' | 'late' | 'excused', modifiedBy: string, reason?: string): boolean;
}
export interface IAttendanceModel extends Model<IAttendanceDocument> {
    markAttendance(teacherId: string, classId: string, subjectId: string, date: Date, period: number, attendanceData: IMarkAttendanceData[]): Promise<IAttendanceDocument>;
    getClassAttendance(classId: string, date: Date, period?: number): Promise<IAttendanceDocument[]>;
    getStudentAttendance(studentId: string, startDate: Date, endDate: Date): Promise<IAttendanceDocument[]>;
    calculateStudentAttendancePercentage(studentId: string, startDate: Date, endDate: Date): Promise<number>;
    getAttendanceStats(schoolId: string, startDate: Date, endDate: Date): Promise<IAttendanceStats>;
