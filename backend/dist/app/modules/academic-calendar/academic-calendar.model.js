...
const specificAudienceSchema = new mongoose_1.Schema({
    grades: {
        type: [Number],
        validate: {
...
const academicCalendarSchema = new mongoose_1.Schema({
    ...
    startTime: {
        ...
        required: function () { return !this.isAllDay; },
    },
    endTime: {
        type: String,
...
academicCalendarSchema.methods.isUpcoming = function () {
    const now = new Date();
    return this.startDate > now;
};
academicCalendarSchema.methods.isOngoing = function () {
    const now = new Date();
    return this.startDate <= now && this.endDate >= now;
};
academicCalendarSchema.methods.isPast = function () {
    const now = new Date();
    return this.endDate < now;
};
academicCalendarSchema.methods.getDuration = function () {
    const timeDiff = this.endDate.getTime() - this.startDate.getTime();
    return Math.ceil(timeDiff / (1000 * 60 * 60 * 24));
};
academicCalendarSchema.methods.getDaysUntilEvent = function () {
    const now = new Date();
    const eventDate = this.startDate;
    const timeDiff = eventDate.getTime() - now.getTime();
    return Math.ceil(timeDiff / (1000 * 60 * 60 * 24));
};
academicCalendarSchema.methods.isTargetedTo = function (userType, userId, grade, section) {
...
academicCalendarSchema.methods.getFormattedDateRange = function () {
    const options = {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
...
academicCalendarSchema.methods.needsReminder = function (days) {
    return this.reminderDays.includes(days) && this.getDaysUntilEvent() === days;
};
academicCalendarSchema.statics.findBySchool = function (schoolId) {
    return this.find({ schoolId, isActive: true })
        .populate('createdBy', 'firstName lastName')
        .sort({ startDate: 1 });
};
academicCalendarSchema.statics.findUpcoming = function (schoolId, days = 30) {
    const now = new Date();
...
academicCalendarSchema.statics.findByDateRange = function (schoolId, startDate, endDate) {
    return this.find({
        schoolId,
        isActive: true,
        $or: [
...
academicCalendarSchema.statics.findByEventType = function (schoolId, eventType) {
    return this.find({ schoolId, eventType, isActive: true })
        .populate('createdBy', 'firstName lastName')
        .sort({ startDate: 1 });
};
academicCalendarSchema.statics.findForUser = function (schoolId, userType, userId, grade, section) {
    const query = {
        schoolId,
        isActive: true,
        $or: [
...
academicCalendarSchema.statics.getMonthlyCalendar = async function (schoolId, year, month) {
    ...
    events.forEach(event => {
        monthlyCalendar.summary[event.eventType]++;
...
academicCalendarSchema.statics.getHolidays = function (schoolId, startDate, endDate) {
    return this.find({
        schoolId,
        eventType: 'holiday',
        isActive: true,
...
academicCalendarSchema.statics.getEventsNeedingReminders = function () {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const maxReminderDays = new Date();
    maxReminderDays.setDate(today.getDate() + 30);
    return this.find({
        isActive: true,
...
academicCalendarSchema.pre('save', function (next) {
    ...
    if (this.reminderDays && this.reminderDays.length > 0) {
        this.reminderDays.sort((a, b) => b - a);
    }
    next();
});
academicCalendarSchema.set('toJSON', {
    transform: function (doc, ret) {
        ret.id = ret._id;
        ret.duration = doc.getDuration();
        ret.daysUntilEvent = doc.getDaysUntilEvent();
        ret.formattedDateRange = doc.getFormattedDateRange();
        if (doc.isUpcoming()) {
            ret.status = 'upcoming';
        }
...
exports.AcademicCalendar = (0, mongoose_1.model)('AcademicCalendar', academicCalendarSchema);
//# sourceMappingURL=academic-calendar.model.js.map