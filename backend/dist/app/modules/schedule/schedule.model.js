...
const schedulePeriodSchema = new mongoose_1.Schema({
    periodNumber: {
        type: Number,
...
scheduleSchema.methods.getTotalPeriodsCount = function () {
    return this.periods.length;
};
scheduleSchema.methods.getPeriodByNumber = function (periodNumber) {
    return this.periods.find(period => period.periodNumber === periodNumber);
};
scheduleSchema.methods.getPeriodsForTeacher = function (teacherId) {
    return this.periods.filter(period => !period.isBreak && period.teacherId && period.teacherId.toString() === teacherId);
};
scheduleSchema.methods.getPeriodsForSubject = function (subjectId) {
    return this.periods.filter(period => !period.isBreak && period.subjectId && period.subjectId.toString() === subjectId);
};
scheduleSchema.methods.hasConflict = function (teacherId, periodNumber) {
    return this.periods.some(period => !period.isBreak &&
        period.teacherId &&
        period.teacherId.toString() === teacherId &&
        period.periodNumber === periodNumber);
};
scheduleSchema.methods.getBreakPeriods = function () {
    return this.periods.filter(period => period.isBreak);
};
scheduleSchema.methods.getClassPeriods = function () {
    return this.periods.filter(period => !period.isBreak);
};
scheduleSchema.methods.calculateDuration = function (startTime, endTime) {
...
scheduleSchema.statics.findBySchool = function (schoolId) {
...
scheduleSchema.statics.findByClass = function (schoolId, grade, section) {
...
scheduleSchema.statics.findByTeacher = function (teacherId) {
    return this.find({
...
scheduleSchema.statics.findBySubject = function (subjectId) {
    return this.find({
...
scheduleSchema.statics.checkTeacherConflict = async function (teacherId, dayOfWeek, _periodNumber, startTime, endTime, excludeScheduleId) {
    let teacherObjectId;
    try {
        teacherObjectId = new mongoose_1.Types.ObjectId(teacherId);
    }
    catch {
        return false;
    }
...
scheduleSchema.statics.generateWeeklySchedule = async function (schoolId, grade, section) {
    ...
    schedules.forEach(schedule => {
        const daySchedule = schedule.periods
            ...
            if (!period.isBreak && period.subjectId && period.teacherId) {
                periodResponse.subject = {
                    id: period.subjectId?._id?.toString() || period.subjectId.toString(),
                    name: period.subjectId?.name || 'Unknown Subject',
                    code: period.subjectId?.code || 'N/A'
                };
                periodResponse.teacher = {
                    id: period.teacherId?._id?.toString() || period.teacherId.toString(),
                    userId: period.teacherId?.userId?._id?.toString() || 'Unknown',
                    teacherId: period.teacherId?.teacherId || 'Unknown',
                    fullName: `${period.teacherId?.userId?.firstName || 'Unknown'} ${period.teacherId?.userId?.lastName || 'Teacher'}`
                };
                const teacherId = period.teacherId?._id?.toString() || period.teacherId.toString();
                const subjectId = period.subjectId?._id?.toString() || period.subjectId.toString();
                if (!teacherMap.has(teacherId)) {
                    teacherMap.set(teacherId, {
                        id: teacherId,
                        name: periodResponse.teacher.fullName,
                        subjects: new Set(),
                        totalPeriods: 0
                    });
                }
                teacherMap.get(teacherId).subjects.add(period.subjectId?.name || 'Unknown Subject');
                ...
                subjectMap.get(subjectId).teachers.add(periodResponse.teacher.fullName);
...
scheduleSchema.statics.getTeacherWorkload = async function (teacherId) {
...
scheduleSchema.statics.calculateDuration = function (startTime, endTime) {
    const [startHour, startMin] = startTime.split(':').map(Number);
    const [endHour, endMin] = endTime.split(':').map(Number);
    const startMinutes = startHour * 60 + startMin;
    const endMinutes = endHour * 60 + endMin;
...
scheduleSchema.pre('save', async function (next) {
    for (const period of this.periods) {
        if (!period.isBreak && period.teacherId) {
            const hasConflict = await this.constructor.checkTeacherConflict(period.teacherId.toString(), this.dayOfWeek, period.periodNumber, period.startTime, period.endTime, this._id?.toString());
            if (hasConflict) {
...
scheduleSchema.set('toJSON', {
    transform: function (doc, ret) {
        ret.id = ret._id;
        ret.className = `Grade ${ret.grade} - Section ${ret.section}`;
        ret.totalPeriods = doc.getTotalPeriodsCount();
        delete ret._id;
        delete ret.__v;
        return ret;
    },
