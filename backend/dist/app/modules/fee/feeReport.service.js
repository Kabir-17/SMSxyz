"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const studentFeeRecord_model_1 = __importDefault(require("./studentFeeRecord.model"));
const feeTransaction_model_1 = __importDefault(require("./feeTransaction.model"));
const feeDefaulter_model_1 = __importDefault(require("./feeDefaulter.model"));
const fee_interface_1 = require("./fee.interface");
const mongoose_1 = require("mongoose");
const Student = (0, mongoose_1.model)("Student");
class FeeReportService {
    async getFinancialOverview(schoolId, academicYear, startDate, endDate) {
        const { Types } = require('mongoose');
        const schoolObjectId = new Types.ObjectId(schoolId);
        const dateFilter = startDate && endDate
            ? { createdAt: { $gte: startDate, $lte: endDate } }
            : {};
        const collectedResult = await feeTransaction_model_1.default.aggregate([
            {
                $match: {
                    school: schoolObjectId,
                    transactionType: fee_interface_1.TransactionType.PAYMENT,
                    status: fee_interface_1.TransactionStatus.COMPLETED,
                    ...dateFilter,
                },
            },
            {
                $group: {
                    _id: null,
                    totalCollected: { $sum: "$amount" },
                },
            },
        ]);
        const totalCollected = collectedResult[0]?.totalCollected || 0;
        const dueResult = await studentFeeRecord_model_1.default.aggregate([
            {
                $match: {
                    school: schoolObjectId,
                    academicYear,
                },
            },
            {
                $group: {
                    _id: null,
                    totalDue: { $sum: "$totalDueAmount" },
                },
            },
        ]);
        const totalDue = dueResult[0]?.totalDue || 0;
        const waivedResult = await studentFeeRecord_model_1.default.aggregate([
            {
                $match: {
                    school: schoolObjectId,
                    academicYear,
                },
            },
            {
                $unwind: "$monthlyPayments",
            },
            {
                $match: {
                    "monthlyPayments.waived": true,
                },
            },
            {
                $group: {
                    _id: null,
                    totalWaived: { $sum: "$monthlyPayments.dueAmount" },
                },
            },
        ]);
        const totalWaived = waivedResult[0]?.totalWaived || 0;
        const totalDefaulters = await feeDefaulter_model_1.default.countDocuments({
            school: schoolObjectId,
        });
        const monthlyBreakdown = await feeTransaction_model_1.default.aggregate([
            {
                $match: {
                    school: schoolObjectId,
                    transactionType: fee_interface_1.TransactionType.PAYMENT,
                    status: fee_interface_1.TransactionStatus.COMPLETED,
                    ...dateFilter,
                },
            },
            {
                $group: {
                    _id: "$month",
                    collected: { $sum: "$amount" },
                },
            },
            {
                $sort: { _id: 1 },
            },
            {
                $project: {
                    month: "$_id",
                    collected: 1,
                    _id: 0,
                },
            },
        ]);
        const gradeWiseBreakdown = await studentFeeRecord_model_1.default.aggregate([
            {
                $match: {
                    school: schoolObjectId,
                    academicYear,
                },
            },
            {
                $group: {
                    _id: "$grade",
                    collected: { $sum: "$totalPaidAmount" },
                    due: { $sum: "$totalDueAmount" },
                    studentCount: { $sum: 1 },
                },
            },
            {
                $sort: { _id: 1 },
            },
            {
                $project: {
                    grade: "$_id",
                    collected: 1,
                    due: 1,
                    studentCount: 1,
                    _id: 0,
                },
            },
        ]);
        const recentTransactions = await feeTransaction_model_1.default.find({
            school: schoolObjectId,
            ...dateFilter,
        })
            .populate("student", "studentId firstName lastName grade")
            .populate("collectedBy", "name email")
            .sort({ createdAt: -1 })
            .limit(10);
        const expectedRevenueResult = await studentFeeRecord_model_1.default.aggregate([
            {
                $match: {
                    school: schoolObjectId,
                    academicYear,
                },
            },
            {
                $group: {
                    _id: null,
                    totalExpected: { $sum: "$totalFeeAmount" },
                },
            },
        ]);
        const totalExpectedRevenue = expectedRevenueResult[0]?.totalExpected || 0;
        return {
            overview: {
                totalExpectedRevenue,
                totalCollected,
                totalDue,
                totalWaived,
                totalDefaulters,
                collectionPercentage: totalExpectedRevenue > 0 ? (totalCollected / totalExpectedRevenue) * 100 : 0,
            },
            monthlyBreakdown,
            gradeWiseBreakdown,
            recentTransactions,
        };
    }
    async getDefaultersReport(schoolId, options) {
        await feeDefaulter_model_1.default.syncDefaultersForSchool(schoolId);
        const query = { school: schoolId };
        if (options?.grade) {
            query.grade = options.grade;
        }
        if (options?.minAmount) {
            query.totalDueAmount = { $gte: options.minAmount };
        }
        if (options?.minDays) {
            query.daysSinceFirstDue = { $gte: options.minDays };
        }
        const defaulters = await feeDefaulter_model_1.default.find(query)
            .populate({
            path: "student",
            select: "studentId firstName lastName grade parentContact parentEmail",
        })
            .sort({ daysSinceFirstDue: -1, totalDueAmount: -1 })
            .limit(options?.limit || 50);
        const stats = await feeDefaulter_model_1.default.aggregate([
            {
                $match: query,
            },
            {
                $group: {
                    _id: null,
                    totalDefaulters: { $sum: 1 },
                    totalDueAmount: { $sum: "$totalDueAmount" },
                    avgDaysSinceFirstDue: { $avg: "$daysSinceFirstDue" },
                },
            },
        ]);
        const criticalDefaulters = await feeDefaulter_model_1.default.find({
            ...query,
            $or: [
                { daysSinceFirstDue: { $gte: 60 } },
                { totalDueAmount: { $gte: 50000 } },
            ],
        }).countDocuments();
        return {
            defaulters,
            statistics: stats[0] || {
                totalDefaulters: 0,
                totalDueAmount: 0,
                avgDaysSinceFirstDue: 0,
            },
            criticalDefaulters,
        };
    }
    async getCollectionByCollector(schoolId, startDate, endDate) {
        return feeTransaction_model_1.default.aggregate([
            {
                $match: {
                    school: schoolId,
                    transactionType: fee_interface_1.TransactionType.PAYMENT,
                    status: fee_interface_1.TransactionStatus.COMPLETED,
                    createdAt: { $gte: startDate, $lte: endDate },
                },
            },
            {
                $group: {
                    _id: "$collectedBy",
                    totalAmount: { $sum: "$amount" },
                    transactionCount: { $sum: 1 },
                },
            },
            {
                $lookup: {
                    from: "users",
                    localField: "_id",
                    foreignField: "_id",
                    as: "collector",
                },
            },
            {
                $unwind: "$collector",
            },
            {
                $project: {
                    collectorId: "$_id",
                    collectorName: "$collector.name",
                    collectorEmail: "$collector.email",
                    totalAmount: 1,
                    transactionCount: 1,
                    _id: 0,
                },
            },
            {
                $sort: { totalAmount: -1 },
            },
        ]);
    }
    async getPaymentTrends(schoolId, academicYear, groupBy = "month") {
        const dateGrouping = groupBy === "month"
            ? {
                year: { $year: "$createdAt" },
                month: { $month: "$createdAt" },
            }
            : groupBy === "week"
                ? {
                    year: { $year: "$createdAt" },
                    week: { $week: "$createdAt" },
                }
                : {
                    year: { $year: "$createdAt" },
                    month: { $month: "$createdAt" },
                    day: { $dayOfMonth: "$createdAt" },
                };
        return feeTransaction_model_1.default.aggregate([
            {
                $match: {
                    school: schoolId,
                    transactionType: fee_interface_1.TransactionType.PAYMENT,
                    status: fee_interface_1.TransactionStatus.COMPLETED,
                },
            },
            {
                $group: {
                    _id: dateGrouping,
                    totalAmount: { $sum: "$amount" },
                    transactionCount: { $sum: 1 },
                },
            },
            {
                $sort: { "_id.year": 1, "_id.month": 1, "_id.day": 1 },
            },
        ]);
    }
    async getFeeCollectionRate(schoolId, academicYear) {
        const feeRecords = await studentFeeRecord_model_1.default.find({
            school: schoolId,
            academicYear,
        });
        const totalStudents = feeRecords.length;
        const totalExpected = feeRecords.reduce((sum, record) => sum + record.totalFeeAmount, 0);
        const totalCollected = feeRecords.reduce((sum, record) => sum + record.totalPaidAmount, 0);
        const totalDue = feeRecords.reduce((sum, record) => sum + record.totalDueAmount, 0);
        const collectionRate = totalExpected > 0
            ? (totalCollected / totalExpected) * 100
            : 0;
        const statusBreakdown = {
            paid: feeRecords.filter((r) => r.status === fee_interface_1.PaymentStatus.PAID).length,
            partial: feeRecords.filter((r) => r.status === fee_interface_1.PaymentStatus.PARTIAL).length,
            pending: feeRecords.filter((r) => r.status === fee_interface_1.PaymentStatus.PENDING).length,
            overdue: feeRecords.filter((r) => r.status === fee_interface_1.PaymentStatus.OVERDUE).length,
        };
        return {
            totalStudents,
            totalExpected,
            totalCollected,
            totalDue,
            collectionRate: Math.round(collectionRate * 100) / 100,
            statusBreakdown,
        };
    }
    async getMonthlyCollectionComparison(schoolId, currentYear, previousYear) {
        const getMonthlyData = async (year) => {
            return feeTransaction_model_1.default.aggregate([
                {
                    $match: {
                        school: schoolId,
                        transactionType: fee_interface_1.TransactionType.PAYMENT,
                        status: fee_interface_1.TransactionStatus.COMPLETED,
                    },
                },
                {
                    $group: {
                        _id: "$month",
                        amount: { $sum: "$amount" },
                    },
                },
                {
                    $sort: { _id: 1 },
                },
            ]);
        };
        const [currentYearData, previousYearData] = await Promise.all([
            getMonthlyData(currentYear),
            getMonthlyData(previousYear),
        ]);
        return {
            currentYear: currentYearData,
            previousYear: previousYearData,
            comparison: currentYearData.map((current) => {
                const previous = previousYearData.find((p) => p._id === current._id);
                const growth = previous
                    ? ((current.amount - previous.amount) / previous.amount) * 100
                    : 0;
                return {
                    month: current._id,
                    currentYearAmount: current.amount,
                    previousYearAmount: previous?.amount || 0,
                    growth: Math.round(growth * 100) / 100,
                };
            }),
        };
    }
    async exportTransactionsCSV(schoolId, startDate, endDate) {
        const transactions = await feeTransaction_model_1.default.find({
            school: schoolId,
            createdAt: { $gte: startDate, $lte: endDate },
        })
            .populate("student", "studentId firstName lastName grade")
            .populate("collectedBy", "name email")
            .sort({ createdAt: -1 });
        return transactions.map((t) => ({
            transactionId: t.transactionId,
            receiptNumber: t.receiptNumber || "",
            date: t.createdAt.toISOString().split("T")[0],
            studentId: t.student?.studentId || "",
            studentName: t.student
                ? `${t.student.firstName} ${t.student.lastName}`
                : "",
            grade: t.student?.grade || "",
            type: t.transactionType,
            amount: t.amount,
            paymentMethod: t.paymentMethod || "",
            collectedBy: t.collectedBy?.name || "",
            status: t.status,
            remarks: t.remarks || "",
        }));
    }
}
exports.default = new FeeReportService();
//# sourceMappingURL=feeReport.service.js.map