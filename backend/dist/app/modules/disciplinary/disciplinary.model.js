...
disciplinaryActionSchema.methods.escalate = async function () {
...
disciplinaryActionSchema.methods.getEscalationLevel = function () {
    ...
    return escalationLevels[this.severity] || 'Unknown';
};
disciplinaryActionSchema.methods.notifyParents = async function () {
    return true;
};
disciplinaryActionSchema.methods.notifyStudent = async function () {
    return true;
};
...
disciplinaryActionSchema.statics.getStudentDisciplinaryHistory = async function (studentId) {
    ...
    return {
        ...
        categoryBreakdown: {
            ...
            discipline: actions.filter((a) => a.category === 'discipline').length,
...
disciplinaryActionSchema.statics.getDisciplinaryStats = async function (schoolId, filters = {}) {
    ...
    return {
        ...
        bySeverity: {
            ...
            medium: stats?.severityBreakdown.filter((s) => s === 'medium').length || 0,
            high: stats?.severityBreakdown.filter((s) => s === 'high').length || 0,
            critical: stats?.severityBreakdown.filter((s) => s === 'critical').length || 0,
        },
        byCategory: {
            behavior: stats?.categoryBreakdown.filter((c) => c === 'behavior').length || 0,
            attendance: stats?.categoryBreakdown.filter((c) => c === 'attendance').length || 0,
            academic: stats?.categoryBreakdown.filter((c) => c === 'academic').length || 0,
            discipline: stats?.categoryBreakdown.filter((c) => c === 'discipline').length || 0,
            uniform: stats?.categoryBreakdown.filter((c) => c === 'uniform').length || 0,
            other: stats?.categoryBreakdown.filter((c) => c === 'other').length || 0,
        },
        byGrade: [],
        recentTrends: []
    };
