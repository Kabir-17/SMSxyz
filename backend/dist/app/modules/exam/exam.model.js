...
exports.ExamResult = exports.Exam = void 0;
const mongoose_1 = require("mongoose");
const gradingScaleSchema = new mongoose_1.Schema({
    gradeA: {
        type: Number,
...
const examResultSchema = new mongoose_1.Schema({
    ...
    marksObtained: {
        type: Number,
        required: function () { return !this.isAbsent; },
...
examSchema.methods.getDurationInHours = function () {
...
examSchema.methods.isOngoing = function () {
    const now = new Date();
...
examSchema.methods.isCompleted = function () {
    const now = new Date();
    const examDateTime = new Date(this.examDate);
...
examSchema.methods.getFormattedTimeSlot = function () {
    return `${this.startTime} - ${this.endTime}`;
};
examSchema.methods.canPublishResults = function () {
    return this.isCompleted() && !this.resultsPublished;
};
examSchema.methods.getGradeFromMarks = function (marks) {
    ...
    if (marks >= scale.gradeA)
        return 'A';
    if (marks >= scale.gradeB)
        return 'B';
    if (marks >= scale.gradeC)
        return 'C';
    if (marks >= scale.gradeD)
        return 'D';
    return 'F';
};
examSchema.methods.getPercentage = function (marks) {
    return Math.round((marks / this.totalMarks) * 100);
};
examSchema.methods.getEligibleStudents = async function () {
...
examSchema.statics.findBySchool = function (schoolId) {
...
examSchema.statics.findByClass = function (schoolId, grade, section) {
    const query = { schoolId, grade, isActive: true };
    if (section)
...
examSchema.statics.findUpcoming = function (schoolId, days = 30) {
    const now = new Date();
...
examSchema.statics.findByDateRange = function (schoolId, startDate, endDate) {
    return this.find({
        schoolId,
        isActive: true,
...
examSchema.statics.getExamSchedule = async function (schoolId, grade, section, startDate, endDate) {
    let exams = await this.findByClass(schoolId, grade, section);
    if (startDate && endDate) {
        exams = exams.filter(exam => exam.examDate >= startDate && exam.examDate <= endDate);
    }
    const schedule = {
        grade,
        section,
    ...
    exams.forEach(exam => {
        ...
        const subjectId = exam.subjectId?._id?.toString() || exam.subjectId.toString();
        ...
        if (exam.isUpcoming())
            schedule.summary.upcomingExams++;
        else if (exam.isOngoing())
            schedule.summary.ongoingExams++;
        else
            schedule.summary.completedExams++;
        if (exam.isPublished)
            schedule.summary.publishedExams++;
    });
...
examSchema.statics.getExamStats = async function (schoolId, examId) {
    const exam = await this.findById(examId).populate('subjectId', 'name');
    if (!exam) {
        throw new Error('Exam not found');
    }
    const ExamResult = (0, mongoose_1.model)('ExamResult');
    ...
    const gradeDistribution = Object.entries(gradeCount).map(([grade, count]) => ({
        ...
        marksRange: this.constructor.getGradeRange(grade, exam.gradingScale, exam.totalMarks)
    }));
...
examSchema.statics.getGradeRange = function (grade, gradingScale, totalMarks) {
    if (!gradingScale) {
...
examResultSchema.pre('save', async function (next) {
    if (!this.isAbsent && this.marksObtained !== undefined) {
        const exam = await (0, mongoose_1.model)('Exam').findById(this.examId);
        if (exam) {
...
examSchema.set('toJSON', {
    transform: function (doc, ret) {
        ret.id = ret._id;
        ret.durationHours = doc.getDurationInHours();
        ret.timeSlot = doc.getFormattedTimeSlot();
        ret.canPublishResults = doc.canPublishResults();
        if (doc.isUpcoming()) {
            ret.status = 'upcoming';
        }
