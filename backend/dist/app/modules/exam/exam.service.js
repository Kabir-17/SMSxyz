...
class ExamService {
    ...
    async getExamById(id, userId, userRole) {
        ...
        if (['teacher', 'admin', 'superadmin'].includes(userRole)) {
            ...
            const submissionCount = await exam_model_1.ExamResult.countDocuments({ examId: exam._id });
    ...
    async updateExam(id, data, userId) {
        ...
        const updatedExam = await exam_model_1.Exam.findByIdAndUpdate(id, updateData, { new: true })
            .populate('teacherId', 'userId teacherId')
            .populate('subjectId', 'name code')
            .populate('schoolId', 'name')
            .populate('createdBy', 'firstName lastName');
